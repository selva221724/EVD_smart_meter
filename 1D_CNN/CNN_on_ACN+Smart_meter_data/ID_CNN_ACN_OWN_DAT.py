...
data_path = r"C:\Users\TamilS\Documents\Python Scripts\EV\EV DETECTION\CNN\synthetic_data_set\Train_and_Test\synth_data - Copy.csv"
sourceData = pd.read_csv(data_path)

# sourceData['time_stamp'] = pd.to_datetime(sourceData['time_stamp'])

X = sourceData[['current_added']]  # independent Variable
Y = sourceData[['target_label']]  # target variable

# split the data into train and test
trainX, testX = train_test_split(X, test_size=0.2, shuffle=False)
trainY, testY = train_test_split(Y, test_size=0.2, shuffle=False)
...
results.plot(title='Actual vs Predicted Time Series - EV Detection ( ACN + SMART METER )')
plt.plot(normalized)
plt.legend(['y true', 'y pred', 'Current(normalized)'], loc="upper right")
